...
export default function PlayerBar({ guildId, snap, pos }: { guildId: string; snap: PlayerSnapshot; pos: number }) {
    ...
    <div className="flex h-20 items-center gap-4 border-t border-border bg-surface px-4">
      ...
      <div className="flex flex-[2] flex-col items-center gap-1">
        <div className="flex items-center gap-2">
          <button className="icon-btn" onClick={() => setLoop(guildId, LOOP_NEXT[snap.loop ?? "off"])} title={`loop: ${snap.loop}`}>
          ...
          <button className="flex h-10 w-10 items-center justify-center rounded-full text-white disabled:opacity-50"
            ...
            onClick={() => (snap.paused ? resume(guildId) : pause(guildId))} disabled={!cur}>
            {snap.paused ? <IconPlay /> : <IconPause />}
          </button>
          <button className="icon-btn" onClick={() => skip(guildId)} disabled={!cur}><IconSkip className="h-4 w-4" /></button>
          <button className="icon-btn" onClick={() => shuffle(guildId)} title="shuffle"><IconShuffle className="h-4 w-4" /></button>
        </div>
        <div className="flex w-full max-w-md items-center gap-2 text-[11px] text-muted">
          ...
          <input type="range" min={0} max={len} value={Math.min(pos, len)} disabled={!cur}
            onChange={(e) => seek(guildId, Number(e.target.value))} className="flex-1 accent-accent2" />
          <span>{fmt(len)}</span>
        </div>
      </div>
      <div className="hidden flex-1 items-center justify-end gap-2 sm:flex">
        ...
        <input type="range" min={0} max={150} defaultValue={snap.volume ?? 100}
          onMouseUp={(e) => setVolume(guildId, Number((e.target as HTMLInputElement).value))} className="w-24 accent-accent2" />
      </div>
    </div>
  );
}
